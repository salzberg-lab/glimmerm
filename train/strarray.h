...
//  using the lambda values in  L .
  ...
  {
   long int  i, j, Ct, Prev_i, Prev_i_Start, Prev_Size;
...
//  Convert the values in this  String_Array  to probabilities.
  ...
  {
   double  Sum;
   long int  i, j, Ct, Prev_Size, Prev_Start, Prev_Sub, Size, Start;
...
//  delta counts in  D .

  {
   ...
   int  Len;
   long int  Prev_Start, Prev_Sub, Size, Sub;
   long int  i, j, Ct, Prev_i, Prev_i_Start, Prev_Size, Used;
...
//  is set to  1.0 .

  {
   ...
   for  (i = 0;  i < Num_Entries;  i ++)
     {
      First = Max = Combine ((* L_Plus [Len]) [i], (* L_Minus [Len]) [i]);
      Sub = Len;
      for  (k = Len - 1;  k >= 0;  k --)
        {
