...
const int  MAX_DATA_LEN = 1000000;
...
{ 
   ...
   int *B;
   long i;
   long String_Len, D_len, Name_len, Poz;
   double Score,S1,S2;
   double score1,score2,score3;
   tree *tdon, *tacc;
   char line[5000];
   int istacc, istdon;
   double thr_acc, thr_don;
   ...
   /* read gene by gene sequences from file */

   while  (fgets (D, MAX_DATA_LEN, Infile) != NULL)
     {
       ...
       for(i=80+Name_len+1;i<D_len-82;i++) {

	 if(D[i]=='a' && D[i+1]=='g') { /* if there is a potential acceptor */
	   ...
	   if(Score>=thr_acc) fprintf(Outfile,"ag: %ld %f\n",i-Name_len,Score);
	   fflush(stdout);
