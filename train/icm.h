...
int TOTAL_NODES;
int MAX_DEPTH;
int MODEL_READ = FALSE;     /* TRUE iff model has been read in, FALSE otherwise */
...
void   Read_Scoring_Model (FILE *fptr);
tModel *   Read_NonCoding_Model (FILE *fptr);

const int INTERVAL = MODEL_LEN;
 ...
 *********************************************************/
tModel *
Read_NonCoding_Model (FILE *fptr)
{
  tModel *NCModel;
...
Read_Scoring_Model (FILE *fptr)
{
  ...
  */
  ...
  MODEL = (tModel **) Safe_malloc (sizeof (tModel *) * 3);
  for (i = 0; i < 3; i++)
    MODEL[i] = (tModel *) Safe_calloc (TOTAL_NODES, sizeof (tModel)); 
  ...
  while (fread (&num_node, sizeof (int), 1, fptr) != 0)
  {
    ...
    /* read in the probabilities */
    if (fread (MODEL[offset][num_node].prob, sizeof (float), 4, fptr) == 0)
    {
      fprintf (stderr, "ERROR reading probs from binary file\n");
      exit (0);
    }
    /* read in the max m.i. pos */
    if (fread (&(MODEL[offset][num_node].mut_info_pos), sizeof (int), 1, fptr) == 0)
    {
      fprintf (stderr, "ERROR reading m.i. pos from binary file\n");
      exit (0);
    }
    ...
    if (num_node != 0 && (num_node-1) != prev_node)
      for (i = prev_node+1; i < num_node; i++)
        MODEL[offset][i].mut_info_pos = -1;
    
    if (num_node == 0 && offset != 0)
      for (i = prev_node+1; i < TOTAL_NODES; i++)
        MODEL [offset-1][i].mut_info_pos = -1;
